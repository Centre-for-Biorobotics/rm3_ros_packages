...
//Platform specific configurations would be here
...
public:
    // BNO080();
    // BNO080(uint8_t address, std::string bus);

	bool begin(uint8_t address = BNO080_DEFAULT_ADDRESS, std::string bus = "/dev/i2c-1", uint8_t intPin = 255); //By default use the default I2C addres, and use Wire port, and don't declare an INT pin
...
private:
    ...
    uint8_t _busNumber;     // Which i2c bus?
