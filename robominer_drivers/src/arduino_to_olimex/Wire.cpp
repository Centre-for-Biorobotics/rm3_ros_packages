...
{	
	//if(::use_debug_mode && ::params_initialized) { printf("TwoWire::begin(): Using pre-defined bus name %s\n",busName); }
...
{	
	...
	if (file < 0) {
		...
		if(::use_debug_mode && ::params_initialized) { printf("     TwoWire::begin(): Error opening I2C bus %s.\n",_busName); }
	}	
	snprintf(busName, 19, "%s", _busName);
	if(::use_debug_mode && ::params_initialized) { printf("     TwoWire::begin(): I2C bus is now open: %s\n",busName); }
}
...
bool TwoWire::selectSlave(uint8_t address)
{
	if (ioctl(file, I2C_SLAVE, address) < 0) {
		/* ERROR HANDLING; you can check errno to see what went wrong */
		if(::use_debug_mode && ::params_initialized) { printf("     TwoWire::selectSlave(): Error opening slave device 0x%02X.\n", address); }
		return false;
	}	
...
int TwoWire::endTransmission(bool b)
{
	...
	if(b)
	{
		if(::use_debug_mode && ::params_initialized) { printf("     TwoWire::endTransmission(): Warning: I2C stop condition is not supported on this platform.\n"); }
	}
	...
	{		
		...
		if (bytesWritten != bytesToSend) {
            /*
			if(::use_debug_mode && ::params_initialized) { printf("     TwoWire::endTransmission(): Error writing to slave 0x%02X: Unexpected number of bytes.\n",slaveAddress); }
			if(::use_debug_mode && ::params_initialized) { printf("       bytesToSend: %ld\n  bytesWritten: %ld\n",bytesToSend,bytesWritten); }
			if(::use_debug_mode && ::params_initialized) { printf("       errno: %s\n",strerror(errno)); }
            */
			ret = 1;
		}	
	}
...
uint8_t TwoWire::read(void)
{
	if(bufIndex >= bufSize)
	{
		if(::use_debug_mode && ::params_initialized) { printf("     TwoWire::read(): Error reading from buffer: index exceeds amount of data currently in buffer.\n"); }
		bufIndex = 0;
	}	
	bufIndex++;
...
{	
	...
	if(quantity > BUFFER_LENGTH){
		if(::use_debug_mode && ::params_initialized) { printf("TwoWire::requestFrom(): Error: Too many bytes requested. Please increase BUFFER_LENGTH.\n"); }
	...
	if(r != quantity)
	{
	  //if(::use_debug_mode && ::params_initialized) { printf("     TwoWire::requestFrom(): Error reading slave device 0x%02X: Unexpected number of bytes.\n",address); }
	  //if(::use_debug_mode && ::params_initialized) { printf("       bytes expected: %d\n  bytes read: %d\n",quantity, r); }
	  //if(::use_debug_mode && ::params_initialized) { printf("       errno: %s\n",strerror(errno)); }
	  return 0;
	}
...
void TwoWire::end(void)
{
	...
	{
		close(file);
		if(::use_debug_mode && ::params_initialized) { printf("     TwoWire::end(): Bus closed\n"); }
	}
}
